...
2. **复制密钥信息**
   - `SecretId`：从控制台复制您的SecretId（格式：以AKID开头的字符串）
   - `SecretKey`：从控制台复制您的SecretKey（格式：一串字符）
...
3. **添加以下Secrets**：
   ...
   **Secret 1：`TENCENT_SECRET_ID`**
   ...
   - Value: 从腾讯云API密钥管理页面获取的SecretId
   - 获取地址：https://console.cloud.tencent.com/cam/capi

   **Secret 2：`TENCENT_SECRET_KEY`**
   - Name: `TENCENT_SECRET_KEY`
   - Value: 从腾讯云API密钥管理页面获取的SecretKey
   - 获取地址：https://console.cloud.tencent.com/cam/capi

   **⚠️ 重要：请使用您的真实密钥，不要使用示例或占位符**
